...
2. Run:
   ...
   ```
   It relaunches itself elevated (UAC prompt) if it isn't already — writing to a physical disk needs Administrator, and doing that check up front means you never lose anything you've already filled in.
3. Under step 1, click **"Download latest (verified)"** — always fetches whatever Debian's `current` release is, and checks it against Debian's own published SHA256SUMS automatically as part of the download, no separate step. Or browse to an ISO you already have and click **"Verify checksum"** to check that one instead. The flash button stays disabled until verification passes (or you explicitly tick "Skip verification (offline)").
4. Under step 2, click **"Download latest Ventoy"** — fetches the latest release straight from [github.com/ventoy/Ventoy](https://github.com/ventoy/Ventoy), verified against the checksum GitHub publishes alongside it (same treatment as the ISO — Ventoy2Disk.exe runs with disk-write access, so it's checked automatically, no override needed). "ventoy.net (manual)" is there if you'd rather download it yourself.
5. **Only genuine USB removable drives are listed** — the tool asks Windows which disks are on the USB bus, so it can't be pointed at an internal drive by mistake. Pick your stick from the list and click through the final "erase this disk, are you sure" warning.
...
`power-api.py` (17823) and `camera-stream-server.py` (17824) both listen on every network interface, not just the tablet itself, so anything on your LAN — Home Assistant included, e.g. a REST sensor or an mjpeg camera you point at the tablet yourself — can reach them. `07-power-drawer.sh` generates a shared token at `/opt/ha-kiosk/api.token` for exactly that: anything calling either service from another machine needs to send it (`Authorization: Bearer <token>`, or `?token=<token>` for the mjpeg URL, which can't send custom headers); requests from the tablet itself (the drawer, the setup screen, the MQTT bridge) never need it. Deleting `/opt/ha-kiosk/api.token` (and not setting `KIOSK_API_TOKEN`/`CAMERA_STREAM_TOKEN`) reverts to the old open-LAN behavior.
