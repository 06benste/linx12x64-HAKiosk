...
with sftp.file("/tmp/_wrap80.sh", "w") as f:
    f.write(
        r"""#!/bin/bash
...
print('WRAP', m.WRAP_X, 'WIDTH', m.WIDTH)
...
open('/tmp/wrap80.jpg','wb').write(data)
print('snap', len(data), list(data[:3]))
PY
"""
    )
sftp.chmod("/tmp/_wrap80.sh", 0o755)
sftp.close()
chan = c.get_transport().open_session()
chan.settimeout(55)
chan.get_pty()
chan.exec_command(f"echo {PASS} | sudo -S -p '' bash /tmp/_wrap80.sh")
buf = b""
t = time.time() + 55
while time.time() < t:
    if chan.recv_ready():
...
sftp = c.open_sftp()
data = sftp.file("/tmp/wrap80.jpg", "rb").read()
sftp.close()
c.close()
(OUT / "wrap80_final.jpg").write_bytes(data)
